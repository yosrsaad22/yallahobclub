// Input component extends from shadcnui - https://ui.shadcn.com/docs/components/input
'use client';
import * as React from 'react';
import { cn } from '@/lib/utils';
import { useMotionTemplate, useMotionValue, motion } from 'framer-motion';

export interface InputProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {}

const AceternityButton = React.forwardRef<HTMLButtonElement, InputProps>(({ className, type, ...props }, ref) => {
  const radius = 100; // change this to increase the rdaius of the hover effect
  const [visible, setVisible] = React.useState(false);

  let mouseX = useMotionValue(0);
  let mouseY = useMotionValue(0);

  function handleMouseMove({ currentTarget, clientX, clientY }: any) {
    let { left, top } = currentTarget.getBoundingClientRect();

    mouseX.set(clientX - left);
    mouseY.set(clientY - top);
  }
  return (
    <motion.div
      style={{
        background: useMotionTemplate`
        radial-gradient(
          ${visible ? radius + 'px' : '0px'} circle at ${mouseX}px ${mouseY}px,
          hsl(var(--primary)),
          transparent 80%
        )
      `,
      }}
      onClick={(event) => event.preventDefault()}
      onMouseMove={handleMouseMove}
      onMouseEnter={() => setVisible(true)}
      onMouseLeave={() => setVisible(false)}
      className="group/input rounded-lg p-[2px] transition duration-300">
      <button
        className={cn(
          `placeholder-text-neutral-600 duration-400 flex h-10 w-full rounded-md
          bg-input px-3 
          py-2
          text-sm
          text-foreground transition
           file:border-0
           file:bg-transparent
           file:text-sm
           file:font-medium
           autofill:border-2
           autofill:border-primary
           autofill:shadow-[inset_0_0_0px_1000px_hsl(var(--input))]
           focus-visible:outline-none
           focus-visible:ring-[2px]
           focus-visible:ring-neutral-600 disabled:cursor-not-allowed disabled:opacity-50
           `,
          'flex items-center',
          className,
        )}
        ref={ref}
        {...props}
      />
    </motion.div>
  );
});
AceternityButton.displayName = 'Input';

export { AceternityButton };
