...
interface DailySubOrdersAndCoursesProps {
  data: DailyProfit[];
}
...
const chartConfig: ChartConfig = {
  subOrders: {
    label: 'SubOrders',
    color: 'hsl(var(--chart-1)/100%)', // Full opacity for stroke
  },
  soldCourses: {
    label: 'SoldCourses',
    color: 'hsl(var(--chart-2)/100%)', // Full opacity for stroke
  },
} satisfies ChartConfig;

export function DailySubOrdersAndCoursesChart({ data }: DailySubOrdersAndCoursesProps) {
    ...
    <Card className="h-full">
      <CardHeader className="p-3">
        <CardTitle>
          <div className="flex flex-col justify-start gap-1">
            ...
            {tStats('daily-sub-orders-courses-title')}
          </div>
        </CardTitle>
        <CardDescription>{tStats('daily-sub-orders-courses-description')}</CardDescription>
      </CardHeader>
      <CardContent className=" p-3">
        {data.length === 0 ? (
          ...
          <ChartContainer className="-ml-9" config={chartConfig}>
            <ResponsiveContainer>
              <BarChart data={data}>
                ...
                <Bar
                  dataKey="subOrders"
                  fill={`${chartConfig.subOrders.color!.replace('/100%', '/70%')}`}
                  stroke={chartConfig.subOrders.color}
                  strokeWidth={2}
                  radius={[4, 4, 0, 0]}
                />
                <Bar
                  dataKey="soldCourses"
                  fill={`${chartConfig.soldCourses.color!.replace('/100%', '/70%')}`}
                  stroke={chartConfig.soldCourses.color}
                  strokeWidth={2}
                  radius={[4, 4, 0, 0]}
                />
              </BarChart>
