...
export function AddProductForm({}: AddProductFormProps) {
  ...
  const [productSizes, setProductSizes] = React.useState<sizeOptions[]>([]);

  const colors: colorOptions[] = Object.values(colorOptions);
  const [productColors, setProductColors] = React.useState<colorOptions[]>([]);
  ...
  const onSubmit: SubmitHandler<schemaType> = async (data, event) => {
    ...
    startTransition(() => {
      addProduct(data).then((res: ActionResponse) => {
        if (res.success) {
          toast({
            variant: 'success',
